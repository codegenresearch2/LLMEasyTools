...
def get_function_schema(function: Union[Callable, LLMFunction], case_insensitive: bool=False, strict: bool=False) -> dict:
    schema = function.schema if isinstance(function, LLMFunction) else {'name': function.__name__.lower() if case_insensitive else function.__name__, 'description': function.__doc__.strip() if function.__doc__ else ''}
    model_json_schema = parameters_basemodel_from_function(function).model_json_schema()
    if strict:
        schema['parameters'] = to_strict_json_schema(model_json_schema)
        schema['strict'] = True
    else:

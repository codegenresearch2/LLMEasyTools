...
def process_tool_call(tool_call, functions_or_models, fix_json_args=True, case_insensitive=False) -> ToolResult:
    """
    Processes a tool call based on the provided tool call and a list of functions or models.
    
    Args:
        tool_call (ChatCompletionMessageToolCall): The tool call to be processed.
...
def split_string_to_list(s: str) -> list[str]:
    """
    Attempts to parse a string as a JSON array. If it fails, it splits the string by commas.
    
    Args:
        s (str): The string to parse.
    
    Returns:
        list[str]: A list of strings parsed from the input.
    """
    try:
        return json.loads(s)
...
def _process_unpacked(function, tool_args={}, fix_json_args=True):
    """
    Processes the arguments of a function call and invokes the function with the processed arguments.
    ...
    Returns:
        tuple: A tuple containing the result of the function call and any soft errors encountered.
    """
...
def _is_list_type(annotation):
    """
    Checks if the given annotation represents a list type.
    
    Args:
        annotation: The annotation to check.
    
    Returns:
        bool: True if the annotation represents a list type, False otherwise.
    """
...
def process_message(
    ...
    Args:
        ...
        fix_json_args (bool): Whether to attempt to fix JSON decoding errors in arguments.
        case_insensitive (bool): Whether to perform a case-insensitive match for tool names.
        executor (ThreadPoolExecutor, optional): An executor to run tool calls in parallel.
